...
rto_actions = [
    ...
    ("on_init", "Serviceable"),
    ("on_confirm", RET10_FULFILLMENT_STATE["PENDING"]),
    ("on_status", RET10_FULFILLMENT_STATE["PACKED"]),
    ("on_status", RET10_FULFILLMENT_STATE["AGENT_ASSIGNED"]),
    ("on_status", RET10_FULFILLMENT_STATE["PICKED_UP"]),
    ("on_status", RET10_FULFILLMENT_STATE["RTO_INITIATED"]),
    ("on_status", RET10_FULFILLMENT_STATE["RTO_DELIVERED"]),
    ("on_cancel", RET10_FULFILLMENT_STATE["CANCELLED"]),
]

print("=== STARTING MERCHANT SIDE RTO & PARTIAL ORDER CANCELLATION DRY-RUN AUDIT ===")

all_passed = True
created_at = _now()

for idx, (action, state_code) in enumerate(rto_actions, start=1):
...
if all_passed:
    print("\n🎉 ALL MERCHANT RTO & PARTIAL CANCELLATION PAYLOADS PASSED RET10 1.2.0 VALIDATION WITH 0 ERRORS!")
else:
    print("\n❌ SOME MERCHANT RTO PAYLOADS FAILED RET10 VALIDATION.")
